function [phi, phi_old, B_up, B_dn, parents, w, O, S_z, E, W, W_bp] = stepwlk(phi, phi_old, B_up, B_dn, parents, N_wlk, N_sites, N, w, O, S_z, E, W, W_bp, H_k, Proj_k_half_up, Proj_k_half_dn, flag_mea, flag_begin_bp, flag_bp, flag_bbp, Phi_T, N_up, N_par, t_bp, t_pop, U, U_imp, Lx, Ly, fac_norm, aux_fld, ind_parents, ind_end, list)
    ...
    sz=zeros(N_wlk,N_sites); % Array containing a given operator
    ...
    for i_wlk=1:N_wlk
        ...
        if w(i_wlk)>0
            ...
            [Phi, w(i_wlk), O(i_wlk), invO_matrix_up, invO_matrix_dn]=halfK(Phi, w(i_wlk), O(i_wlk), Proj_k_half_up, Proj_k_half_dn, Phi_T, N_up, N_par);
            if w(i_wlk)>0
            ...
            if w(i_wlk)>0
                ...
                if w(i_wlk)>0
                    ...
                    if flag_mea==1
                        [e(i_wlk)]=measure(H_k, Phi(:,:), Phi_T,  invO_matrix_up, invO_matrix_dn, N_up, N_par, U, U_imp);
                    end
                    % measure a backpropagated observable if needed
                    if flag_bp==1
                        % Selects right father
                        i_father=parents(i_wlk, ind_end);
                        [sz(i_wlk,:)]=measure_bp(Phi_T, phi_old(:,:,i_father,ind_end), B_up(:,i_wlk,:, ind_end), B_dn(:,i_wlk,:, ind_end), Proj_k_half_up, Proj_k_half_dn, t_bp, t_pop, N_up, N_par, N_sites);
                    end
                end
            end
        end
