...
H_k(1,1)=H_k(1,1)+0.5*U_imp;
% the matrix of the operator exp(-deltau*K/2)
Proj_k_half_up = expm(-0.5*deltau*H_k); 
H_k(1,1)=H_k(1,1)-U_imp;
Proj_k_half_dn = expm(-0.5*deltau*H_k);
...
E_T = E_K+E_V+U_imp*0.5*(n_r_up(1)-n_r_dn(1));
E_T=real(E_T);
